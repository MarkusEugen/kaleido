...
def process_video(in_path, out_path, n_strips=7, gap=8, out_height=None,
    ...
    def _bg_detect():
        ...
        while True:
            ...
            if new_curves is None:
                ...
                curves = _repair_curves(new_curves)
    ...
    def _bg_extract():
        while True:
            ...
            strips = []
            for s in range(n_strips):
                band = _extract_band(frame, curves[s], strip_width, led_half_w)
                band = band[y_top:y_top + out_height]
    ...
    try:
        # The extraction thread signals _DONE onto encode_q when it finishes;
        # wait for the encoder thread to drain, then wind everything down.
        threads[-1].join()   # encoder finished
    finally:
